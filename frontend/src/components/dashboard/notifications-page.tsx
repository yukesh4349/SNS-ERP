...
export function NotificationsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"history" | "general" | "classwise">("history");
          ...
          {!isTeacher ? (
            <>
              ...
              <div className="lg:col-span-7">
                  ...
                  <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                    {uniqueHistory.length > 0 ? uniqueHistory.map((item) => (
                      ...
                      <div className="py-12 text-center">
                        <p className="text-xs font-bold text-slate-300 uppercase tracking-widest">No Broadcast History Found</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* 1. General Notification Inline Form */}
                {activeTab === "general" && (
                  <motion.div
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: -20 }}
                    className="bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-sm flex flex-col gap-6"
                  >
                ...
                {/* 2. Class-wise Notification Inline Form */}
                {activeTab === "classwise" && (
                  <motion.div
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
